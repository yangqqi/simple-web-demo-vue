                    ...
                    <van-list v-model="isLoading" :finished="isFinished" finished-text="没有更多了" @load="onButtomLoad">
                        <a class="weui-media-box weui-media-box_appmsg" v-for="(item, index) in resList" :key="index" @click.stop="onClick(item)">
                            <div class="weui-media-box__bd">
                                ...
                                <p class="weui-media-box__desc">仓库：{{ item.stockName }}</p>
                            </div>
                        </a>
                    </van-list>
                </div>
...
export default {
    ...
    methods: {
        loadList() {
            ...
            this.isLoading = true;
            this.$systemAxios
                .get('/api/scm/base/goodsshelves/query', { params })
                .then(res => {
                    if (res.code !== 0) {
        ...
        handleDelete(id) {
            ...
            this.$weui.confirm(
                ...
                function() {
                    ...
                    _that.$systemAxios
                        .delete(`/api/scm/base/goodsshelves/del/${id}`)
                        .then(res => {
